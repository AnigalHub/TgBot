...
async function outputMessageOnCommand(message:string, chatId:number, bot:TelegramBot):Promise<boolean>{
    ...
    if(message == '/messageOptions'){
        await bot.sendMessage(chatId,'<b><u>Варианты написания сообщений:\n</u></b>' +
            ...
            '31)<code>в секунд/минут/часов «число месяц год (число слово число)»\n\n</code>' +
            '32)<code>в «Х(число цифрой)» секунд/минут/часов «число месяц год (число слово число)»\n\n</code>' +
            '33)<code>в «Х(число словами)» секунд/минут/часов «число месяц год (число слово число)»\n\n</code>',  {parse_mode: 'HTML'}
        )
        return true
    }
    return false
