...
class FutureTimeAndMessage{
    private chatId:number
    private array:Array<string>
    private dateMessage:Date
    ...
    async CalculationsAndHandlingErrorsOnInputIn( keywordInMessage:number, timeMessage:number, messageFuture:string, millisecondsTime:number){
        let arrayElementAfterKeyword1 = this.array[keywordInMessage+1] // элемент массива после ключевого слова - первый
        let arrayElementAfterKeyword2 = this.array[keywordInMessage+2] // элемент массива после ключевого слова - второй
        let arrayElementAfterKeyword3 = this.array[keywordInMessage+3] // элемент массива после ключевого слова - третий
        let arrayElementAfterKeyword4 = this.array[keywordInMessage+4] // элемент массива после ключевого слова - четвертый
        ...
        if(/^[0-9]*$/.test(arrayElementAfterKeyword1)) { // только цифры
            ...
            if(time > 24 && convertTime.ConvertTimeToMilliseconds(arrayElementAfterKeyword2,1) >= 3600000){
                await bot.sendMessage(this.chatId, 'Ошибка! Время не может быть больше 24');
            }
                ...
                await bot.sendMessage(this.chatId, 'Ошибка! Некорректно введено время. Пример: 10 сек | 15 минут | 9 часов');
            }
            else if(!arrayElementAfterKeyword3){
                await bot.sendMessage(this.chatId, 'Ошибка! Не указана дата');
            }
            else{
                if (/[А-яЁё]/.test(arrayElementAfterKeyword3)){ // только буквы
                    if((arrayElementAfterKeyword3) == "в" || (arrayElementAfterKeyword3) == "во"){
                        ...
                        if (dayOfTheWeek.SearchForTheDayNumberOfTheWeek() != -1){
                            ...
                            millisecondsTime = futureDateAndTime.getTime() - this.dateMessage.getTime()
                            messageFuture = this.array.slice((keywordInMessage+5),this.array.length).join(' ')//сообщение, которое напоминаем
                            setTimeout(() => bot.sendMessage(this.chatId, messageFuture),millisecondsTime);// функция со временем - когда напомнить + сообщение - что напоминаем
                            DateAsString(millisecondsTime,this.dateMessage)
                        }
                    }
                    else{
                        ...
                        if((time < this.dateMessage.getHours()) && arrayElementAfterKeyword3 == 'сегодня'){
                            await bot.sendMessage(this.chatId,'Ошибка! Время указано которое уже прошло - напомнить невозможно');
                        }
                        else if(convertTime.ConvertTimeToMilliseconds(arrayElementAfterKeyword3,1) >= 3600000){
                            await bot.sendMessage(this.chatId, 'Ошибка! Некорректно введено время и дата - неизвестно когда напоминать');
                        }
                        else if(futureDay  == -1){
                            await bot.sendMessage(this.chatId,'Ошибка! Некорректно введена дата. Время указано, а дата нет.');
                        }
                        else{
                            let futureDate = new Date(this.dateMessage.getFullYear(), this.dateMessage.getMonth(), futureDay)
                            millisecondsTime = convertTime.CountDifferenceInMillisecondsBetweenFutureAndCurrentDates(timeMessage,futureDate.getHours(),time,this.array,keywordInMessage+2)

                            messageFuture = this.array.slice((keywordInMessage+4),this.array.length).join(' ')//сообщение, которое напоминаем
                            setTimeout(() => bot.sendMessage(this.chatId, messageFuture),millisecondsTime);// функция со временем - когда напомнить + сообщение - что напоминаем
                            DateAsString(millisecondsTime,this.dateMessage)
                        }
                    }
                    ...
                    if  (this.array[keywordInMessage + 3][2] != this.array[keywordInMessage + 3][5] &&
                        ...
                        await bot.sendMessage(this.chatId, 'Ошибка! Некорректно введена дата. Опечатка в дате!');
                    }
                    else {
                        let yearMessage
                ...
                if(arrayElementAfterKeyword2 == "в"){
                    if (/^[А-яЁё]*$/.test(arrayElementAfterKeyword3)){
                        if(convertTime.ConvertTimeToMilliseconds(arrayElementAfterKeyword3,1) == 0 &&
                            ...
                            await bot.sendMessage(this.chatId, 'Ошибка! Некорректно указано время');
                        }
                        else {
                        ...
                        if (isNaN(time)){
                            await bot.sendMessage(this.chatId, 'Ошибка! Неизвестно время - исправьте ошибку');
                        }
                        else if(time > 24){
                            await bot.sendMessage(this.chatId, 'Ошибка! Время не может быть больше 24');
                        }
...
async function CalculationsAndHandlingErrorsOnInputIn(chatId:number, words:Array<string>, keywordInMessage:number, timeMessage:number, messageFuture:string, millisecondsTime:number){
    let arrayElementAfterKeyword1 = words[keywordInMessage+1] // элемент массива после ключевого слова - первый
    let arrayElementAfterKeyword2 = words[keywordInMessage+2] // элемент массива после ключевого слова - второй
    let arrayElementAfterKeyword3 = words[keywordInMessage+3] // элемент массива после ключевого слова - третий
    let arrayElementAfterKeyword4 = words[keywordInMessage+4] // элемент массива после ключевого слова - четвертый
