...
bot.on('message', async (msg) =>{
    ...
    if(msg.text == '/start'){
        await bot.sendMessage( chatId,'Это <b>Telegram-бот - Напоминальщик событий.<b>' +
            'Бот, который помогает не забыть о событии и напоминает в определенный день и дату пользователю о событии.\n' +
            'Создайте простое напоминание боту, отправив простое сообщение вида: Напомнить «КОГДА (дата и время)» «СООБЩЕНИЕ (что напомнить)»' +
            'Пример: Напомнить в 2 часа 20.12.23 заплатить за квартиру',{parse_mode: 'HTML'}
        )
        return
    }
    if(msg.text == '/help'){
        await bot.sendMessage( chatId,'<b>Варианты написания сообщений:\n</b>' +
            '<pre><b>1)</b>напомнить через секунду/минуту/полчаса/час/день/сутки/неделю/месяц/полгода/год\n</pre>' +
