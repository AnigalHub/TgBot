...
bot.on('message', async (msg) =>{
    ...
    if(msg.text == '/start'){
        await bot.sendMessage( chatId,'<b>Telegram-бот-Напоминальщик событий</b>.\n' +
            ...
            '<b>Правила при написании сообщения:\n</b>' +
            '<b>1)Структура сообщения:</b><pre>Напомнить КОГДА (дата и время) СООБЩЕНИЕ (что напомнить)\n</pre>' +
            ...
            '<pre>-- единица времени: через час|через неделю</pre>' +
            '<b>5)После указателя времени "В" всегда идет:\n</b>' +
            '<pre>-- число и единица времени: в 15 часов|в пять мин</pre>' +
            '<pre>-- единица времени: в час</pre>' +
            '<pre>-- день недели: в пятницу</pre>'
            ,{parse_mode: 'HTML'})
        return
    }
    if(msg.text == '/help'){
        await bot.sendMessage( chatId,'<b>Варианты написания сообщений:\n</b>' +
            ...
            '<pre>10)напомнить через «Х(число словами)» дней/недель/месяцев/лет в секунду/минуту/час\n</pre>' +
